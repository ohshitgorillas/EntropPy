...
- **False trigger detection uses full validation set**: Fixed false trigger checking to use the full validation dictionary instead of the filtered validation set. Previously, typos like "ethre" were incorrectly allowed with NONE boundary because the filtered validation set didn't include words like "urethrectomy" and "brethren" where the typo appears as a substring. Now false trigger checks use the complete English words dictionary, matching QMK's validation, preventing false trigger warnings.
