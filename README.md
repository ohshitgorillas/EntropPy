...
**Version 0.5.1 (Beta)** | [Changelog](CHANGELOG.md)
...
python -m entroppy --debug --verbose --debug-words "the,because" --top-n 1000 --output corrections

# Trace typos with wildcard pattern matching (*) and word boundaries (:)
python -m entroppy --debug --verbose --debug-typos "teh,*toin:,abc*" --top-n 1000 --output corrections
```
...
```bash
python -m entroppy --top-n 5000 --output corrections --reports reports --verbose
```
...
Run with: `python -m entroppy --config settings/config.json`
